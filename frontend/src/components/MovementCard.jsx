...
const CardIcon = ({ icon, text = 'tooltip 💡', action }) => (
  <button
    type="button"
    className="button-icon mt-auto group right-14 left-0"
    onClick={() => action()}
  >
    {icon}
...
const MovementCard = ({
  ...
  <div key={exerId} className="movement-card">
    ...
    <div className="ml-auto mt-auto mr-4 flex flex-col">
      <CardIcon
        ...
        text="Edit movement 💡"
        action={() => {
          triggerModal(ModalTypes.UPDATE, movementId);
        }}
      />
      <CardIcon
        ...
        action={() => {
          triggerModal(ModalTypes.DESTROY, movementId);
        }}
      />
    </div>
  </div>
