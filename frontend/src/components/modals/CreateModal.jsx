...
const CreateModal = ({ triggerModal }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      await axios.post(`${process.env.BASE_BE_URL}/create`, values);
  ...
  return (
    <div className="p-6 space-y-6">
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" disabled={isLoading} className="submit-button">{isLoading ? <FaCircleNotch className="animate-spin-slow" size="28" /> : submitted ? 'Success' : 'Submit'}</button>
        </form>
    </div>
  );
};
